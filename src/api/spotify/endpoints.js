...
export default {
  ...
  getMyProfile: type => spotify({ method: 'get', url: `/me/${type === 'profile' ? '' : type}`, params: { market } }),
  ...
  skipTo: direction => spotify({ method: 'post', url: `/me/player/${direction}`, params: { device_id: deviceId } }),

  // seek playback to position in track
  seekTo: position => spotify({ method: 'put', url: '/me/player/seek', params: { device_id: deviceId, position_ms: position } }),

  // set playback state and track
  setPlayback: (state, uris) => spotify({ method: 'put', url: `/me/player/${state}`, data: { uris }, params: { device_id: deviceId } }),

  // toggle repeat for playback
  toggleRepeat: () => spotify({ method: 'put', url: '/me/player/repeat', params: { state: 'context', device_id: deviceId } }),

  // set shuffle state for playback
  setShuffle: () => spotify({ method: 'put', url: '/me/player/shuffle', params: { state: !shuffleState, device_id: deviceId } }),

  // get the current playback
  getPlayback: () => spotify({ method: 'get', url: '/me/player', params: { market } }),

  // get url to authenticate the user
  getAuthURL: (protocol, host) => backend({ method: 'get', url: '/getAuthURL', params: { redirectURI: `${protocol}//${host}/login` } }),

  // get access token
  getAccessToken: code => backend({ method: 'get', url: '/getToken', params: { code } }),

  // refresh access token
  refreshAccessToken: () => backend({ method: 'get', url: '/refreshToken', params: { token: refreshToken } }),
};
